...
public class _01_Main {
	...
	public static void main(String[] args) {
		...
		/*
		 * 추상이란? - 여러가지 사물이나 개념으로부터 공통이 되는 특성을 파악하는 것을 말한다.
		 * 
		 * [면접] 1. 추상클래스 => 업무표준화, 작업명세서 : 정의된 추상메서드 
		 ...
		 * 	- 다형성 적용 가능하다.
		 * 
		 * - 문법 : public abstract class 클래스명 { public abstract 반환형 메서드명(); // 추상메서드 }
		 * 
		 * 2. 추상메서드 
		 ...
		 * 
		 * 4. 추상클래스의 객체 생성 
		 * 	- 추상클래스는 객체를 생성할 수 없으며(추상 메서드 때문), 상속 계층에서 추상적인 개념을 나타내기 위한 용도로 사용한다. .
		...
		// 부모클래스를 new하면 오류발생 -> 추상메서드를 포함하고 있기 때문
		// _01_Computer com = new _01_Computer(); // 오류 - 추상클래스는 new 불가
		...
		desktop.display(); // 재정의된 자식메서드 호출(자식클래스가 new에 의해 실제 힙메모리 공간을 차지했기 때문)
		desktop.typing(); // 재정의된 자식메서드 호출(자식클래스가 new에 의해 실제 힙메모리 공간을 차지했기 때문)
